...
TEST(detail_test_raw, api)
{
    ...
    std::size_t alignment_padding =
        (8 - (abacus::detail::header_bytes() + max_name_bytes +
              max_metrics * (max_name_bytes + 1))) %
        8;
    std::size_t size =
        abacus::detail::header_bytes() + max_name_bytes + alignment_padding +
        max_metrics * (max_name_bytes + sizeof(uint64_t) + sizeof(uint8_t));
